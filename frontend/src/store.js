...
export const analyze = createAsyncThunk('warrant/analyze', async code => json(await fetch('/api/warrants/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code})})))
export const loadHistory = createAsyncThunk('warrant/history', async () => json(await fetch('/api/history?limit=30')))
export const clearHistory = createAsyncThunk('warrant/clear', async () => json(await fetch('/api/history',{method:'DELETE'})))
